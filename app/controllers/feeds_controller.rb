class FeedsController < ApplicationController
  ...
  def create
    case feed_type_params(params)
      when 'text'
        @feed = @current_user.feed.create feed_type: 'text', comment: comment_params(params)
        render json: { errors: @feed.errors.full_messages }, status: :bad_request and return if @feed.errors.any?
      when 'image'
        begin
          img = Magick::Image.from_blob(File.read image_params(params).tempfile).shift
  ...
  def feed_type_params params
    params.require(:feed_type)
  end
